class QPart {
    ...
    getTreeLevelData() {
        function CountEntitiesByLevel(part: QPart) {
            let s = 0; part.#entities.forEach(e => { if (e instanceof Artefact) s++; })
            levelArtefact[0] += s; levelArtefact[part.level] += s;
            s = 0; part.#entities.forEach(e => { if (e instanceof Fence) s++; })
